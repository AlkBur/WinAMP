...
func Init(w, h int) error {
	...
	//Задание размеров окна консоли
	if w != -1 && h != -1 {
		orig_size = coord{short(w), short(h)}
		set_console_window_info(out, w, h)
		err = set_console_screen_buffer_size(out, orig_size)
		if err != nil {
			return err
		}
	}else {
	...
	Body.Width = int(term_size.x)

	DefaultEvtStream.Init()
	DefaultEvtStream.Merge(NewSysEvtCh())
	DefaultEvtStream.Merge(NewTimerCh(time.Second))
	DefaultEvtStream.Merge(usrEvtCh)

	DefaultEvtStream.Handle(EventResize, func(e Event) {
		Body.Width = e.Width
	})
