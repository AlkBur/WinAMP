...
var (
	...
	proc_set_console_title    		   	  = kernel32.NewProc("SetConsoleTitleW")
...
func SetTitle(title string) bool {
	...
	ret, _, _ := proc_set_console_title.Call(uintptr(unsafe.Pointer(strptr)))
	return (ret != 0)
}
...
func set_console_window_info(out syscall.Handle,  w, h int)  {
	r := small_rect{0, 0, short(w - 1), short(h - 1)}
	proc_set_console_window_info.Call(
		uintptr(out),
		uintptr(1),
